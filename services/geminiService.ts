// Mock geminiService (TS) — usado por las vistas dentro de la carpeta `cita-rd`.
// Reemplazar por implementación real cuando esté listo.

export interface AIProfileFeedback {
  feedback: string;
  improvedBio: string;
  icebreakers: string[];
}

export async function getAIProfileFeedback(bio: string, interests: string[]): Promise<AIProfileFeedback> {
...
export async function getIcebreakerSuggestions(name: string, interests: string[] = []): Promise<string[]> {
